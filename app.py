...
market_choice = st.sidebar.radio(
    ...
    [
        ...
        "🌏 한국주식/미국주식(나스닥)/금/현금 듀얼모멘텀",
        "📊 자산군별 모멘텀 스코어"
    ]
)

# 메뉴 구분을 위한 플래그 (포트폴리오 백테스트 유무)
is_portfolio_mode = "듀얼모멘텀" in market_choice

# 🐛 에러 해결: 단어 포함 여부(in)가 아닌, 메뉴 이름과 정확히 일치(==)하도록 수정
if market_choice == "🇰🇷 한국주식/채권/현금 듀얼모멘텀":
...
elif market_choice == "🇺🇸 미국주식/채권/현금 듀얼모멘텀":
...
elif market_choice == "🌏 한국주식/미국주식/금/현금 듀얼모멘텀":
...
elif market_choice == "🌏 한국주식/미국주식(나스닥)/금/현금 듀얼모멘텀":
    # 새로 추가된 나스닥(QQQ) 듀얼모멘텀 메뉴
    st_title = "📊 한국/미국(나스닥)/금/현금 듀얼모멘텀 전략"
    st_desc = "- **한국주식**: EWY ETF (iShares MSCI South Korea)\n- **미국주식**: QQQ ETF (Invesco QQQ Trust, 나스닥100)\n- **금**: GLD ETF (SPDR Gold Shares)\n- **현금**: 파킹통장 (연 1.6% 단리/복리 적용)"
    tickers = ['EWY', 'QQQ', 'GLD']
    names = ['한국주식', '미국주식(나스닥)', '금', '현금']
    emojis = ['🇰🇷', '🇺🇸', '✨', '💵']
    colors = ['#ef553b', '#1f77b4', '#ffd700', '#636efa']
    info_text = "💡 **전략 작동 원리:** 전통 듀얼모멘텀 전략과는 달리 직전달 말일의 종가와 과거 1~12개월 전 말일의 종가 비교하여 모멘텀 스코어를 계산하므로 좀 더 정교한 투자 비율을 정할수 있습니다. 또한, 주식/채권/금이 모두 하락장이라 0점을 받게 되면, 전체 점수 중 현금만 남게 되어 포트폴리오의 100%가 현금으로 이동하여 방어력이 극대화 됩니다. 본 전략은 각각 자산의 단독투자 대비 샤프지수를 높이기 위한 전략입니다."
    ...
    # '자산군별 모멘텀 스코어' 메뉴
    st_title = "📊 자산군별 모멘텀 스코어"
    st_desc = "- 글로벌 주요 10개 ETF 자산군의 현재 평균모멘텀스코어를 원래 순서대로 비교하여 시장의 큰 추세를 한눈에 파악합니다."
    tickers = ['QQQ', 'SPY', 'EWY', 'EWJ', 'EWZ', 'IEF', 'TLT', 'DBC', 'DBA', 'GLD']
    names = tickers
    info_text = "💡 **메뉴 설명:** 이 화면에서는 특정 포트폴리오 비중이나 백테스트를 계산하지 않습니다. 오직 각 자산이 현재 상승장인지 하락장인지 직관적으로 파악하기 위해 모멘텀 스코어(0.0 ~ 1.0)만 산출합니다. 스코어가 1.0에 가까울수록 강한 상승장, 0.0에 가까울수록 강한 하락장을 의미합니다."
...
# ⚙️ 3. 데이터 로드 및 연산
# ==========================================
df_monthly = load_data(tickers, names, include_cash=is_portfolio_mode)
...
if is_portfolio_mode:
    ...
    tab1, tab2, tab3 = st.tabs(["📊 현재 스코어 비교", "📈 과거 스코어 추이", "🗂️ 상세 데이터베이스"])
    
    with tab1:
        latest_date = avg_scores_df.index[-1].strftime('%Y년 %m월')
        st.subheader(f"🔔 {latest_date} 마지막 거래일 기준 모멘텀 스코어")
        st.info(info_text)
        
        # 순서 1: 스코어 막대 그래프 먼저 표출
        latest_scores = avg_scores_df.iloc[-1]
        ...
        # 순서 2: 분석 대상 ETF 상세 설명 (차트 아래로 이동)
        with st.expander("📌 분석 대상 ETF 상세 설명", expanded=True):
